...
def latex_template(name, title, root): # NOT LATEX. This is actually markdown
    return '\n'.join((
        r"<div style='width: 100%' class='ui rounded images'>",
        rf"<img class='ui image' src='"+root+title+".svg'>",
        r"</div>"))
...
def maybe_recompile_figure(filepath):
    ...
    pyperclip.copy(latex_template(name, beautify(name), filepath))
...
def create(title, root):
    ...
    print(indent(latex_template(figure_path.stem, title, root), indentation=leading_spaces))
