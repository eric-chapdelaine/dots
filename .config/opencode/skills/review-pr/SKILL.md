---
name: review-pr
description: Review a GitHub pull request end-to-end — reads the PR, the linked Jira ticket, and relevant repo context, then produces a structured review categorized by severity and confidence. Never writes to GitHub.
---

## What I do

Given a GitHub PR URL (e.g. `https://github.com/wayfair-shared/dynamic-sequence-engine-gql-server/pull/2890`):

1. Read the PR fully — title, description, all changed files, and all inline diff hunks
2. Extract the Jira ticket ID from the PR description and read that ticket
3. Understand the broader repo context — what the repo does, what patterns it follows, and how the changed files fit in
4. Determine whether the PR actually solves the problem stated in the Jira ticket
...
Use the GitHub MCP (or `gh` CLI) to fetch:
...
Extract the ticket ID from the PR description (look for patterns like `POSTDW-1234`, `SE-5678`, `WIMS-999`, etc.).

Use the Jira MCP to read:
...
Do not skip this step. If the changed code is part of a critical subsystem (e.g. request replay, cold transfer, graph traversal), read the surrounding files to understand the full impact.

### 4. Assess alignment with the ticket

Answer explicitly:
- Does the PR solve the problem described in the Jira ticket?
...
> Things that look suspicious but may be intentional or have context the reviewer lacks. Flag for the author to confirm.
...
<If there is any part of the PR that you could not fully evaluate due to missing context, state it explicitly here. Do NOT mark the PR as good if you have unresolved context gaps. Instead, list exactly what you didn't understand and why it matters.>
...
- **Do not say the PR is good if you have context gaps.** If you could not fully understand what a change does (e.g., because you couldn't read a key dependency, or the business logic is opaque), say so explicitly in "Context Gaps" and set the verdict to "NEEDS MORE CONTEXT".
...
## Jira ticket ID patterns used at Wayfair

Common project keys seen in this codebase: `POSTDW`, `SE`, `WIMS`, `RAP`, `SVC`. The ticket ID is usually in the PR title or the first paragraph of the PR description.
